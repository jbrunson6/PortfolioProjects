...
CREATE TABLE IF NOT EXISTS nash_housing_clean2 AS (
	SELECT 
		unique_id,
		parcel_id,
		CASE
			WHEN land_use = 'SINGLE FAMILY' THEN land_use
			WHEN land_use = 'RESIDENTIAL CONDO' THEN land_use
			WHEN land_use = 'CONDO' THEN 'RESIDENTIAL CONDO'
			WHEN land_use = 'DUPLEX' THEN land_use
			WHEN land_use = 'ZERO LOT LINE' THEN land_use
			WHEN land_use = 'RESIDENTIAL COMBO/MISC' THEN land_use
			WHEN land_use = 'TRIPLEX' THEN land_use
			WHEN land_use = 'CONDOMINIUM OFC OR OTHER COM CONDO' THEN 'RESIDENTIAL CONDO'
...
DROP TABLE IF EXISTS nash_housing_clean2

SELECT DISTINCT land_use
FROM nash_housing_clean2